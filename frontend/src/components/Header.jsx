...
export default function Header({ viewMode, onGoHome, onStartWizard, onOpenAuthModal, onOpenHistoryModal }) {
  ...
  return (
    <header className="border-b border-slate-800/80 bg-[#070a12]/95 backdrop-blur-md sticky top-0 z-50">
      <div className="max-w-6xl mx-auto px-3 sm:px-6 py-3.5 flex items-center justify-between gap-2">
        ...
        >
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-tr from-emerald-500 via-teal-500 to-indigo-500 p-0.5 shadow-lg shadow-emerald-500/20 group-hover:scale-105 transition-transform overflow-hidden shrink-0">
            <img src="/favicon.jpg" alt="Founder's Hub Logo" className="w-full h-full object-cover rounded-[10px]" />
          </div>
          <div>
            <h1 className="text-base sm:text-lg font-extrabold text-white tracking-tight leading-none flex items-center gap-1.5 sm:gap-2">
              <span>Founder's Hub</span>
              <span className="text-[9px] sm:text-[10px] font-mono font-bold bg-emerald-950 text-emerald-300 border border-emerald-800/60 px-1.5 py-0.5 rounded">
                AI Platform
              </span>
            </h1>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {viewMode === 'wizard' ? (
            <button
              type="button"
              onClick={onGoHome}
              className="px-2.5 sm:px-3.5 py-1.5 rounded-xl text-xs font-semibold bg-slate-900 hover:bg-slate-800 text-slate-300 hover:text-white border border-slate-800 transition-all flex items-center gap-1 sm:gap-1.5 cursor-pointer"
            >
            ...
            <button
              ...
              className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs font-extrabold bg-emerald-500 hover:bg-emerald-400 text-slate-950 transition-all flex items-center gap-1 sm:gap-1.5 cursor-pointer shadow-lg shadow-emerald-500/20"
            >
          ...
          {isAuthenticated ? (
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                className="flex items-center gap-1.5 sm:gap-2 bg-slate-900 hover:bg-slate-800 border border-slate-800 hover:border-slate-700 p-1 sm:p-1.5 sm:pr-3 rounded-xl transition-all cursor-pointer"
              >
                {avatarUrl ? (
                  <img
                    src={avatarUrl}
                ...
                <span className="text-xs font-bold text-slate-200 hidden sm:inline-block max-w-[100px] md:max-w-[140px] truncate">
                  {displayName}
                </span>
            ...
            <button
              ...
              className="px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-xl text-xs font-bold bg-slate-900 hover:bg-slate-800 text-indigo-300 hover:text-white border border-indigo-900/60 hover:border-indigo-700/80 transition-all flex items-center gap-1.5 cursor-pointer shadow"
            >
